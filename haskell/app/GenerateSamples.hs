...
saveSample saver path value description =
  saveSampleWithJson saver path value (toJSON value) description
...
main = do
  ...
  saveSample saveSztCompressedAggressive
  ...
  saveSample saveSzt (samplesDir </> "large_json_normal.szt") 
    (JsonArray (replicate 20 exampleJson))
    "Large JSON array without dedup"

  saveSample saveSztCompressedAggressive (samplesDir </> "large_json_aggressive.szt") 
    (JsonArray (replicate 20 exampleJson))
