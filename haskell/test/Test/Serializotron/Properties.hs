...
genResult genE genA = Gen.choice
  [ Error <$> genE
  , Success <$> genA
  ]
...
genList genA = Gen.recursive Gen.choice
  [ pure Nil ]
...
genPerson = Person
  ...
  <*> ((\name -> name <> "@example.com") <$> Gen.text (Range.linear 1 20) Gen.alpha)
  <*> Gen.bool
...
prop_deduplication_preserves_data = property $ do
  ...
  annotate $ "Original size: " ++ show originalSize ++ ", deduplicated: " ++ show dedupedSize 
           ++ ", shared values: " ++ show sharedCount
