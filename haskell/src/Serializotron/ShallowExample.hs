...
generateTopologyCachedId name regions =
  CachedId $ BS.pack $ "Topology:" <> show name <> ":" <>
...
mkTopology name regions = Topology
...
generateDomainCachedId topology subset =
  CachedId $ "Domain:" <> unCachedId (topologyCachedId topology) <> ":" <>
...
mkDomain topology subset = Domain
...
exampleGridTopology = mkTopology "Grid2D" $ HS.fromList
...
exampleNetworkTopology = mkTopology "Network" $ HS.fromList
...
exampleLargeModelWithDuplication = Model
  ...
  where
    ...
    generatePoint i = "(" <> pack (show (i `mod` 4)) <> "," <>
                             pack (show ((i `div` 4) `mod` 4)) <> ")"
